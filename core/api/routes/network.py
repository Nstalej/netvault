        ...
        for entry in mac_table:
            ...
            for other_id, other_data in manager._cache.items():
                ...
                if target_mac in other_macs:
                    edges.append(
                        {"from": f"device_{device_id}", "to": f"device_{other_id}", "label": port, "type": "physical"}
                    )
...
async def search_network(
    mac: Optional[str] = None, ip: Optional[str] = None, manager: DeviceManager = Depends(get_manager)
):
    ...
    for device_id, data in manager._cache.items():
        ...
        if ip and device_ip == ip:
            results.append(
                {"device_id": device_id, "device_name": device_cfg.get("name"), "type": "device_match", "ip": device_ip}
            )
